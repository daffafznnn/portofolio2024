<template>
  <main class="container mx-auto flex min-h-screen w-full items-center justify-center text-white">
    <section class="flex flex-col space-y-10">
      <div class="text-center text-4xl font-bold text-cyan-500">Reset Password</div>
      <span class="text-sm text-center sm:text-wrap">please follow the instructions if you forgot your password</span>
      <div class="sm:w-full w-96 mx-auto focus-within:border-none transform border-b-2 border-cyan-400 bg-transparent text-lg duration-300 focus-within:border-cyan-500">
        <input
          ...
          class="w-full border-none bg-transparent outline-none placeholder:italic placeholder:flex placeholder:justify-end placeholder:text-cyan-400 focus:outline-none"
        />
      </div>
      <div v-if="isVerify" class="space-y-8">
        <div  class="relative sm:w-full w-96 mx-auto focus-within:border-none transform border-b-2 border-cyan-400 bg-transparent text-lg duration-300 focus-within:border-cyan-500">
        <input
          v-model="code"
          type="text"
        ...
        <button 
          ...
          class="absolute right-0 top-0 mt-2 mr-2 text-cyan-400 underline"
        >
          Resend Code
        </button>
        <span v-else class="absolute right-0 top-0 mt-2 mr-2 text-cyan-400 font-mono">
       ...
       <div class="relative sm:w-full w-96 mx-auto focus-within:border-none transform border-b-2 border-cyan-400 bg-transparent text-lg duration-300 focus-within:border-cyan-500" :class="!matchConfPassword ? 'border-red-400' : 'border-cyan-400' ">
      ...
      <button 
        ...
        @click="submitCode" 
        :disabled="loading" 
        class="sm:w-full w-96 mx-auto transform rounded-sm bg-transparent py-2 font-bold duration-300 hover:outline hover:outline-cyan-400 text-cyan-400 border border-cyan-400 border-opacity-30"
      >
      ...
      <a href="/login" class="sm:mx-0 mx-auto flex items-center transform font-semibold text-cyan-500 duration-300 hover:text-gray-200">
        <svg 
          xmlns="http://www.w3.org/2000/svg" 
          width="25" 
          height="25" 
...
export default {
  ...
  methods: {
    ...mapActions('settings', ['forgotPassword', 'verifyCode', 'resendCodeAction']),
    validateEmail() {
      if (!this.email) {
    ...
    validateCode() {
      if (!this.code) {
        this.$message.error("Code cannot be empty!");
        return false;
      }
      return true;
    },
    ...
    async submitCode() {
      try {
        ...
        const success = await this.forgotPassword({
          email: this.email
        });
        if (success) {
          this.isVerify = true;
    ...
    async submitVerifyCode() {
      try {
        ...
        if (!this.validateCode() || !this.validatePasswords()) return;
        ...
        const success = await this.verifyCode(payload);
        if (success) {
          this.isVerify = false;
    ...
    async resendVerificationCode() {
      ...
      try {
        const success = await this.resendCodeAction({
          email: this.email
        });
        if (success) {
