<template>
  <div id="projects">
    <h1 class="py-4 font-bold uppercase">My projects</h1>
    ...
    <div class="flex items-center justify-center overflow-hidden font-sans">
      <div class="w-full overflow-auto">
        <div class="rounded shadow-md">
          <el-skeleton :loading="loading" animated class="animate-pulse">
            <template #template>
              <div v-for="n in 10" :key="n" class="px-6 py-3 border-b border-gray-200">
                <el-skeleton-item variant="text" />
              </div>
            </template>
            <template #default>
              <table class="w-full table-auto min-w-max" v-if="paginatedProjects.length">
                <thead>
                  <tr class="text-sm leading-normal text-white uppercase bg-black/25">
                ...
                <tbody class="text-sm font-light text-gray-200">
                  ...
                  >
                    <td class="px-6 py-3 text-left cursor-pointer whitespace-nowrap">
                    ...
                    <td class="px-6 py-3 text-center">
                      <div class="w-full rounded-full bg-white/10 group-hover:bg-black/25 dark:bg-gray-700">
                    ...
                    <td class="px-6 py-3 text-center ">
                      <div class="flex justify-center cursor-pointer item-center group-hover:text-slate-700">
      ...
      <div v-if="showModal" class="fixed top-0 left-0 flex items-center justify-center w-full h-full py-10 bg-black bg-opacity-50">
          ...
          <div class="w-full h-[38rem] overflow-y-auto">
            <div class="m-8 my-16 max-w-[400px] mx-auto">
              ...
              <form v-if="selectedActions !== 'read'" @submit.prevent="submitProject">
                ...
                <div class="p-2">
                <input type="text" id="title" v-model="form.title" placeholder="title" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm bg-white/10 placeholder:text-cyan-400 focus:ring-cyan-500 focus:border-cyan-500 sm:text-sm" required>
                </div>
                </div>
                <div class="mx-2 sm:mx-auto">
                  <!-- Form untuk menjawab pertanyaan -->
                    <div class="mb-4">
                      ...
                      <textarea v-model="form.description" :disabled="selectedProject.status === 'missed'" id="answer" name="answer" rows="3"  placeholder="description" class="block w-full mt-1 ml-2 border-gray-300 rounded-md shadow-sm bg-white/10 placeholder:text-cyan-400 focus:ring-cyan-500 focus:border-cyan-500 sm:text-sm" required></textarea>
                    </div>
                  </div>
                <div class="mx-2 mb-4 sm:mx-auto">
                  <label for="technologiesUsed" class="block text-sm font-medium text-gray-300">TechnologiesUsed:</label>
                  <div class="p-2">
                  <input type="text" v-model="form.technologiesUsed" id="technologiesUsed" placeholder="technologiesUsed" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm bg-white/10 placeholder:text-cyan-400 focus:ring-cyan-500 focus:border-cyan-500 sm:text-sm" required>
                  </div>
                </div>
                    <div class="mx-2 mb-4 sm:mx-auto">
                  <label for="githubUrl" class="block text-sm font-medium text-gray-300">GithubUrl:</label>
                  <select  v-model="form.githubUrl"  @change="checkOtherOption" class="block w-full mt-1 text-white bg-blue-900 border-gray-300 rounded-md shadow-sm placeholder:text-cyan-400 focus:ring-cyan-500 focus:border-cyan-500 sm:text-sm">
                   ...
                   <input v-if="otherOptionSelected" v-model="form.githubUrl" type="text" placeholder="Enter GitHub URL" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm bg-white/10 placeholder:text-cyan-400 focus:ring-cyan-500 focus:border-cyan-500 sm:text-sm">
                </div>
                <div class="mx-2 mb-4 sm:mx-auto">
                  <label for="websiteUrl" class="block text-sm font-medium text-gray-300">WebsiteUrl:</label>
                  <div class="p-2">
                  <input type="text" v-model="form.websiteUrl" id="websiteUrl" placeholder="websiteUrl" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm bg-white/10 placeholder:text-cyan-400 focus:ring-cyan-500 focus:border-cyan-500 sm:text-sm">
                  </div>
                </div>
                 <div class="mx-2 mb-4 sm:mx-auto">
                  <label for="category" class="block text-sm font-medium text-gray-300">Category:</label>
                  <select v-model="form.categoryId"  class="block w-full mt-1 text-white bg-blue-900 border-gray-300 rounded-md shadow-sm placeholder:text-cyan-400 focus:ring-cyan-500 focus:border-cyan-500 sm:text-sm">
                    <option value="" disabled selected>Select category</option>
                    <option v-for="item in getCategories" :label="item.name" :key="item.id" :value="item.id">{{ item.name }}</option>
                  </select>  
                </div>
                  <div class="mx-2 mb-4 sm:mx-auto">
                  <label for="progress" class="block text-sm font-medium text-gray-300">Progress  <span class="flex items-end justify-end">{{ form.progress }}%</span></label>
                ...
                <div class="mx-2 mb-4 sm:mx-auto">
                    <select v-model="form.status" placeholder="Select Repository" class="block w-full mt-1 text-white bg-blue-900 border-gray-300 rounded-md shadow-sm placeholder:text-cyan-400 focus:ring-cyan-500 focus:border-cyan-500 sm:text-sm">
                   ...
                   <div class="mb-4 sm:mx-auto">
                     <label
                    for="dropzone-file"
                    class="flex flex-col items-center w-full max-w-lg p-6 mx-auto mb-5 text-center border-2 border-blue-400 border-dashed cursor-pointer rounded-xl bg-white/10"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                    ...
                    <h2 class="mt-4 text-xl font-medium tracking-wide text-cyan-400">
                     Project image
                    </h2>

                    <p class="mt-2 tracking-wide text-gray-200">
                   ...
                   <button type="submit" :disabled="loading" :class="{ 'hidden': selectedActions !== 'add' && selectedActions !== 'edit'}" class="inline-flex items-center px-4 py-2 mx-2 text-base font-medium text-white border border-transparent rounded-md bg-cyan-500 hover:bg-cyan-600">
                  ...
                  <!-- <button type="button" @click="deleteQuestionWithUuid(selectedProject)" :class="{ 'hidden': selectedActions !== 'read'}" class="inline-flex items-center px-4 py-2 text-base font-medium text-white bg-red-500 border border-transparent rounded-md hover:bg-red-600">Delete</button> -->
                  </div>
              </form>
              <!-- end form -->
              ...
              <div v-else class="container mx-auto">
                <div class="mx-2 mb-4 sm:mx-auto">
               <img class="object-cover w-full duration-300 ease-in-out rounded-lg max-h-30 transition-filter" alt="featured image" :src="selectedProject.url" loading="lazy" @load="imageLoaded" />
              </div>
               <div class="mx-2 mb-1 sm:mx-auto">
                ...
                Github
               <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="mx-2 bi bi-github" viewBox="0 0 16 16">
               ...
               <div class="mx-2 mb-4 sm:mx-auto">
                ...
                Website
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="mx-2 bi bi-globe" viewBox="0 0 16 16">
              ...
              <div class="mx-2 mb-4 sm:mx-auto">
                  <p class="w-auto p-1 font-sans text-base text-left border rounded-md bg-white/15">{{ selectedProject.description }}</p>
              </div>
              </div>
              <!-- end content -->
                <!-- Tombol untuk menutup modal -->
                <button @click="closeModal" class="w-full p-3 font-semibold border rounded-full bg-white/10 border-cyan-400 text-cyan-400">Close</button>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  try {
    ...
    nextPage() {
     ...
     checkOtherOption() {
      // Set otherGithubUrl to empty if githubUrl is not 'other'
      if (this.form.githubUrl === 'other') {
        this.form.githubUrl = '';
        this.otherOptionSelected =  true
      } else if (this.form.githubUrl !== 'other') {
        this.otherOptionSelected =  false
      }
    },
  }
