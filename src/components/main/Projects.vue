<template>
  <section id="porto" ref="porto" class="py-20">
    ...
    <div v-else class="mx-auto grid max-w-screen-xl grid-cols-1 gap-5 p-5 sm:grid-cols-2 md:grid-cols-3 lg:gap-10">
      <article v-for="item in paginatedProjects" :key="item.id" :id="item.id" :class="{ 'animate-slide-up': isFirstLoad, 'article': true }" class="h-90 col-span-1 m-auto min-h-full cursor-pointer overflow-hidden rounded-sm pb-2 transition-transform duration-500 hover:translate-y-2 relative group">
...
export default {
  ...
  computed: {
    ...
    paginatedProjects() {
      const projects = this.filterProjects(); // Fungsi untuk memfilter dan mengurutkan proyek
  ...
  methods: {
    ...
    // Check if the number of items in the selected category exceeds or is less than itemsPerPage
    const categoryItemCount = filteredProjects.length;
    const hasEnoughItems = categoryItemCount > this.itemsPerPage;

    // Update shouldShowPagination based on whether the category has enough items
    this.shouldShowPagination = hasEnoughItems;

    // Sort the filtered projects by createdAt date
